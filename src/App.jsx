...
CheckCircle, Lock, Mail, ArrowRight, Search, Download,
ChevronLeft, Users, AlertCircle, CheckSquare, Upload, X, Shield, Clock,
Send, Layers, FolderOpen, Receipt, RefreshCw, TrendingDown, Check, Info,
HelpCircle, Activity, PieChart, FileSpreadsheet, FileType, Pen,
ScanSearch, ClipboardList, CreditCard, Scale, Star, BookOpen,
UserCheck, Building, Briefcase, LayoutDashboard, KeyRound, PartyPopper
} from “lucide-react”;
...
if(!data?.access_token) return { data:null, error:{message:"Onjuist e-mailadres of wachtwoord."} };
setAuthToken(data.access_token);
return { data: { user: data.user, session: data }, error: null };
} catch(e) {
return { data:null, error:{message:"Onjuist e-mailadres of wachtwoord."} };
}
}},
...
const F = { display:”‘Cormorant Garamond’,Georgia,serif”, body:”‘Jost’,sans-serif” };

function GlobalStyles() {
useEffect(()=>{
const id=“gds-fonts”; if(document.getElementById(id)) return;
const l=document.createElement(“link”); l.id=id; l.rel=“stylesheet”;
l.href=“https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400&family=Jost:wght@300;400;500;600;700&display=swap”;
document.head.appendChild(l);
},[]);
return <style>{`*,*::before,*::after{box-sizing:border-box;}html,body{margin:0;padding:0;background:#F5F1EC;}body{font-family:'Jost',sans-serif;color:#3A2E28;-webkit-font-smoothing:antialiased;}button,input,textarea,select{font-family:'Jost',sans-serif;}::-webkit-scrollbar{width:4px;}::-webkit-scrollbar-thumb{background:#D6D3CE;border-radius:2px;}@keyframes fadeUp{from{opacity:0;transform:translateY(6px);}to{opacity:1;transform:translateY(0);}}@keyframes pulse{0%,100%{opacity:1;}50%{opacity:.4;}}.fu{animation:fadeUp .26s cubic-bezier(.22,.68,0,1.2) both;}tr:hover td{background:#F0EBE5!important;}.photo-hover:hover{opacity:1!important;}`}</style>;
}
...
const Chip=({label,color,bg})=>(<span style={{fontSize:9,fontWeight:600,letterSpacing:“0.08em”,color,background:bg,padding:“3px 9px”,borderRadius:4,textTransform:“uppercase”,whiteSpace:“nowrap”}}>{label}</span>);
const Badge=Chip;
...
const PageHeader=({kicker,title,action})=>(
  ...
  <div style={{marginBottom:22,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
    <div>
      <div style={{fontSize:9,fontWeight:600,letterSpacing:"0.14em",color:C.muted,textTransform:"uppercase",marginBottom:5}}>{kicker}</div>
      <h1 style={{fontFamily:F.display,fontSize:28,fontWeight:600,color:C.text,margin:0,lineHeight:1}}>{title}</h1>
    </div>
    {action}
  </div>
);
const SideSection=({label})=>(<div style={{padding:"14px 12px 4px",fontSize:8,fontWeight:600,color:C.taupeLight,letterSpacing:"0.14em",textTransform:"uppercase"}}>{label}</div>);
const SideBtn=({icon:Icon,label,isActive,onClick,danger,badge})=>(
  <button onClick={onClick} style={{width:"100%",display:"flex",alignItems:"center",gap:9,padding:"8px 12px",borderRadius:8,border:"none",cursor:"pointer",textAlign:"left",background:isActive?C.crimsonFaint:"transparent",color:isActive?C.crimson:danger?"#EF4444":C.secondary,marginBottom:1,transition:"all .15s"}}>
    {Icon&&<Icon size={13} strokeWidth={isActive?2.4:1.7} style={{flexShrink:0}}/>}
    <span style={{fontSize:11,fontWeight:isActive?600:400,flex:1}}>{label}</span>
    {badge&&<span style={{fontSize:8.5,fontWeight:700,background:C.crimson,color:CREAM,padding:"2px 6px",borderRadius:10,minWidth:18,textAlign:"center"}}>{badge}</span>}
    {isActive&&<div style={{width:5,height:5,borderRadius:"50%",background:C.crimson,flexShrink:0}}/>}
  </button>
);
const Pill=({label,active,onClick})=>(<button onClick={onClick} style={{padding:"5px 14px",borderRadius:20,border:`1.5px solid ${active?C.crimson:C.border}`,background:active?C.crimson:"transparent",color:active?CREAM:C.secondary,fontSize:11,fontWeight:600,cursor:"pointer",transition:"all .15s"}}>{label}</button>);
function Toast({msg,onClose}){useEffect(()=>{const t=setTimeout(onClose,3200);return()=>clearTimeout(t);},[]);return(<div className="fu" style={{position:"fixed",bottom:24,right:24,zIndex:2000,background:C.espresso,color:CREAM,borderRadius:12,padding:"12px 18px",display:"flex",alignItems:"center",gap:10,boxShadow:"0 8px 30px rgba(58,46,40,.35)",fontSize:12,fontWeight:500}}><Check size={14} color={C.green}/>{msg}<button onClick={onClose} style={{background:"none",border:"none",color:C.taupeLight,cursor:"pointer",marginLeft:8,padding:0}}><X size={13}/></button></div>);}
...
const STATUS_COLOR={Actief:C.green,Prioriteit:C.crimson,“In Review”:C.indigo,Geblokkeerd:C.red,“Wacht op Cliënt”:C.amber,Gepauzeerd:C.secondary,Gesloten:C.mushroom};
...
return(
<aside style={{width:224,background:C.surface,borderRight:`1px solid ${C.border}`,display:“flex”,flexDirection:“column”,flexShrink:0,overflowY:“auto”}}>
<div style={{padding:“16px 14px 12px”,borderBottom:`1px solid ${C.border}`,display:“flex”,alignItems:“center”,gap:10}}>
<div style={{width:34,height:34,borderRadius:9,background:C.crimson,display:“flex”,alignItems:“center”,justifyContent:“center”,flexShrink:0}}>
<span style={{color:CREAM,fontWeight:800,fontSize:14,fontFamily:F.display}}>GE</span>
</div>
<div>
<div style={{fontSize:11,fontWeight:700,color:C.text,lineHeight:1.2}}>{user.role===“client”?“The Glass Executive”:“Vitreous Executive”}</div>
<div style={{fontSize:8.5,color:C.secondary,letterSpacing:“0.07em”,textTransform:“uppercase”,marginTop:1}}>{user.role===“client”?“Cliëntportaal”:“Command Center”}</div>
</div>
</div>
...
return(
<header style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:“0 22px”,display:“flex”,alignItems:“center”,height:54,flexShrink:0,gap:14,position:“relative”,zIndex:100}}>
{/* Global search */}
<div style={{position:“relative”,flex:1,maxWidth:380}} onMouseDown={e=>e.stopPropagation()}>
...
style={{width:“100%”,padding:“7px 32px 7px 32px”,borderRadius:8,border:`1.5px solid ${showSearch&&q?C.crimson:C.border}`,fontSize:12,outline:“none”,background:C.bg,color:C.text,transition:“border-color .15s”}}
/>
{q&&<button onClick={()=>{setQ(””);setResults([]);}} style={{position:“absolute”,right:10,top:“50%”,transform:“translateY(-50%)”,background:“none”,border:“none”,cursor:“pointer”,color:C.secondary,padding:0}}><X size={13}/></button>}
{/* Search dropdown */}
{showSearch&&q&&(
<div style={{position:“absolute”,top:“calc(100% + 6px)”,left:0,right:0,background:C.surface,borderRadius:12,boxShadow:“0 8px 32px rgba(58,46,40,.18)”,border:`1px solid ${C.border}`,overflow:“hidden”,zIndex:200}}>
{results.length===0?(
<div style={{padding:“16px 16px”,fontSize:12,color:C.secondary,textAlign:“center”}}>Geen resultaten voor “{q}”</div>
):results.map((r,i)=>(
<div key={i} onClick={()=>{setView(r.view);setQ(””);setResults([]);setShowSearch(false);}} style={{padding:“10px 14px”,display:“flex”,alignItems:“center”,gap:10,cursor:“pointer”,borderTop:i>0?`1px solid ${C.border}`:“none”}}>
<div style={{width:30,height:30,borderRadius:7,background:`${r.color}18`,display:“flex”,alignItems:“center”,justifyContent:“center”,flexShrink:0}}>
...
<div style={{fontSize:10,color:C.secondary}}>{r.type} · {r.sub}</div>
</div>
<ChevronRight size={12} color={C.secondary}/>
</div>
))}
{results.length>0&&<div style={{padding:“8px 14px”,borderTop:`1px solid ${C.border}`,fontSize:9,fontWeight:700,color:C.muted,letterSpacing:“0.08em”,textTransform:“uppercase”}}>Druk Enter om te zoeken</div>}
</div>
)}
</div>
...
```
  <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8}}>
    ...
    <div style={{display:"flex",border:`1px solid ${C.border}`,borderRadius:7,overflow:"hidden"}}>
      {["NL","EN"].map(l=>(<button key={l} onClick={()=>setLanguage(l)} style={{padding:"4px 9px",fontSize:10,fontWeight:700,border:"none",cursor:"pointer",background:language===l?C.crimson:"transparent",color:language===l?CREAM:C.secondary,transition:"all .15s"}}>{l}</button>))}
    </div>
...
<main style={{flex:1,overflow:“auto”,padding:“22px 26px”}} key={view+(detailEng?.id||””)}>
...
{[
{label:t(“healthLabel”),Icon:AlertTriangle,value:“04”,sub:t(“criticalThresholds”),c:C.crimson,badge:t(“actionRequired”)},
{label:t(“exposureLabel”),Icon:BarChart3,value:“12”,sub:t(“riskyDossiers”),c:C.amber,badge:t(“revision”)},
{label:t(“speedLabel”),Icon:CheckCircle,value:“89%”,sub:t(“onTrack”),c:C.green,badge:t(“optimal”)},
].map(({label,Icon,value,sub,c,badge})=>(
<div key={label} style={{background:C.surface,borderRadius:14,padding:“17px 19px”,border:`1px solid ${C.border}`}}>
<div style={{display:“flex”,justifyContent:“space-between”,marginBottom:10}}>
<span style={{fontSize:9,fontWeight:600,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{label}</span>
<Icon size={16} color={c}/>
</div>
<div style={{fontFamily:F.display,fontSize:32,fontWeight:600,color:C.text,lineHeight:1}}>{value}</div>
<div style={{fontSize:11,color:C.secondary,marginTop:4}}>{sub}</div>
<div style={{marginTop:12,paddingTop:9,borderTop:`1px solid ${C.border}`,display:“flex”,justifyContent:“flex-end”}}>
<span style={{fontSize:9,fontWeight:700,color:c,letterSpacing:“0.06em”}}>{badge}</span>
</div>
</div>
))}
</div>
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 260px",gap:14,marginBottom:18}}>
    <div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead><tr style={{background:C.warm50}}>{["NAAM",t("dept"),t("phase"),t("health")].map(h=><th key={h} style={{padding:"8px 18px",textAlign:"left",fontSize:9,fontWeight:700,letterSpacing:"0.09em",color:C.secondary,textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
        <tbody>{eng.slice(0,4).map(e=>(
          <tr key={e.id} onClick={()=>setDetailEng(e)} style={{borderTop:`1px solid ${C.border}`,cursor:"pointer"}}>
            <td style={{padding:"12px 18px"}}><div style={{fontSize:13,fontWeight:600,color:C.text}}>{e.name}</div><div style={{fontSize:10,color:C.secondary}}>{e.ref}</div></td>
            <td style={{padding:"12px 18px"}}><DeptTag dept={e.dept}/></td>
            <td style={{padding:"12px 18px"}}><span style={{fontSize:11,padding:"3px 9px",borderRadius:20,background:C.bg,color:C.text,border:`1px solid ${C.border}`}}>{e.phase}</span></td>
            <td style={{padding:"12px 18px"}}><div style={{display:"flex",alignItems:"center",gap:6}}><HealthDot status={e.health}/><span style={{fontSize:10,fontWeight:700,color:e.health==="red"?C.red:e.health==="amber"?C.amber:C.green}}>{e.health==="red"?t("critical"):e.health==="amber"?t("amber"):t("stable")}</span></div></td>
          </tr>
        ))}</tbody>
      </table>
    </div>
...
<table style={{width:“100%”,borderCollapse:“collapse”}}>
<thead><tr style={{background:C.warm50}}>{[“ENGAGEMENT”,“AFDELING”,“FASE”,“GEZONDHEID”,“MGR”].map(h=><th key={h} style={{padding:“10px 18px”,textAlign:“left”,fontSize:9,fontWeight:700,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{h}</th>)}</tr></thead>
<tbody>{eng.map(e=>(
<tr key={e.id} onClick={()=>setDetailEng(e)} style={{borderTop:`1px solid ${C.border}`,cursor:“pointer”}}>
...
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:“hidden”}}>
<table style={{width:“100%”,borderCollapse:“collapse”}}>
<thead><tr style={{background:C.warm50}}>{[“NAAM”,“CLIËNT”,“FASE”,“GEZONDHEID”,“ACTIE”].map((h,i)=><th key={i} style={{padding:“10px 18px”,textAlign:“left”,fontSize:9,fontWeight:700,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{h}</th>)}</tr></thead>
<tbody>{eng.map(e=>(
...
const phaseMap={TC:TC_PHASES,FF:FF_PHASES};
...
const addTask=()=>{if(!newTaskTitle.trim())return;setLocalTasks(ts=>[…ts,{id:`t${Date.now()}`,title:newTaskTitle,priority:newTaskPrio,due:newTaskDue||”—”,status:“open”,assignee:user.avatar}]);setNewTaskTitle(””);setNewTaskDue(””);setNewTaskPrio(“normal”);setShowTaskForm(false);showToast(“Taak aangemaakt”);};
...
<div onClick={()=>{const extra=p==="Review"?{status:"In Review"}:{};updateEng({phase:p,...extra});showToast(`Fase gewijzigd naar ${p}${p==="Review"?" · Status → In Review":""}`);}}  style={{width:28,height:28,borderRadius:“50%”,display:“flex”,alignItems:“center”,justifyContent:“center”,background:i<phaseIdx?C.crimson:i===phaseIdx?C.crimsonFaint:”#F0EDE8”,border:`2px solid ${i<=phaseIdx?C.crimson:C.border}`,marginBottom:6,cursor:“pointer”,transition:“all .15s”}}>
...
<button key={tb.id} onClick={()=>setActiveTab(tb.id)} style={{flex:1,padding:“8px 12px”,borderRadius:8,border:“none”,cursor:“pointer”,display:“flex”,alignItems:“center”,justifyContent:“center”,gap:7,background:activeTab===tb.id?C.surface:“transparent”,color:activeTab===tb.id?C.text:C.secondary,fontWeight:activeTab===tb.id?700:400,fontSize:12,transition:“all .15s”,boxShadow:activeTab===tb.id?“0 1px 4px rgba(0,0,0,.07)”:“none”}}>
...
<button onClick={()=>setShowTaskForm(v=>!v)} style={{display:“flex”,alignItems:“center”,gap:5,padding:“5px 11px”,borderRadius:7,background:”#6366F1”,color:CREAM,border:“none”,fontSize:10,fontWeight:700,cursor:“pointer”}}><Plus size={11}/> Taak</button>
</div>
{showTaskForm&&(
<div style={{padding:“14px 16px”,background:”#F5F3FF”,borderBottom:`1px solid ${C.border}`,display:“flex”,gap:8,flexWrap:“wrap”}}>
...
{localTasks.length===0&&!showTaskForm&&<div style={{padding:24,textAlign:“center”,color:C.secondary,fontSize:13}}>Geen taken.</div>}
{localTasks.map(tk=>(
<div key={tk.id} style={{padding:“14px 16px”,borderTop:`1px solid ${C.border}`,display:“flex”,alignItems:“center”,gap:12}}>
...
{tk.status===“done”&&<CheckCircle size={11} color={CREAM}/>}
</div>
<div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:tk.status===“done”?C.secondary:C.text,textDecoration:tk.status===“done”?“line-through”:“none”}}>{tk.title}</div><div style={{display:“flex”,alignItems:“center”,gap:10,marginTop:3}}><PriorityDot level={tk.priority}/><span style={{fontSize:10,color:C.secondary}}>{t(“dueDate”)} {tk.due}</span></div></div>
...
{activeTab===“actions”&&(
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:“hidden”}}>
<div style={{padding:“12px 16px”,borderBottom:`1px solid ${C.border}`,display:“flex”,alignItems:“center”,gap:8,background:C.greenBg}}>
<Eye size={13} color={C.green}/><span style={{fontSize:11,fontWeight:700,color:C.green}}>Cliëntacties — Zichtbaar in portaal</span>
</div>
{localActions.map(a=>(
...
<div><div style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:3}}>{a.title}</div><div style={{fontSize:11,color:C.secondary}}>{a.desc}</div></div>
...
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,display:“flex”,flexDirection:“column”,maxHeight:460}}>
...
<button onClick={()=>setVis(v=>!v)} style={{display:“flex”,alignItems:“center”,gap:5,background:vis?C.greenBg:C.warm50,border:`1px solid ${vis?C.green:C.border}`,borderRadius:6,padding:“3px 9px”,cursor:“pointer”,fontSize:10,fontWeight:700,color:vis?C.green:C.secondary,marginBottom:7,transition:“all .15s”}}>
...
useEffect(()=>{
supabase.from("tasks").select("id,title,priority,status,due_date").order("created_at",{ascending:false})
.then(({data})=>{ if(data?.length) setTasks(data.map(tk=>({id:tk.id,title:tk.title,priority:tk.priority||"normal",status:tk.status||"open",due:tk.due_date?new Date(tk.due_date).toLocaleDateString("nl-SR",{day:"2-digit",month:"short"}):"—",assignee:user.avatar}))); })
.catch(()=>{});
},[]);
...
try{await supabase.from("tasks").update({status:newStatus}).eq("id",id);}catch(e){console.warn("toggle:",e.message);}
};
const addTask=async()=>{
if(!newTitle.trim())return;
const localTask={id:`t${Date.now()}`,title:newTitle,priority:newPrio,due:newDue?new Date(newDue).toLocaleDateString("nl-SR",{day:"2-digit",month:"short"}):"—",status:"open",assignee:user.avatar};
setTasks(ts=>[localTask,...ts]);
setNewTitle("");setNewPrio("normal");setNewDue("");setShowForm(false);
try{
const {data:engs}=await supabase.from("engagements").select("id").limit(1);
const engId=engs?.[0]?.id||null;
if(engId){
const {data:saved}=await supabase.from("tasks").insert({title:newTitle.trim(),priority:newPrio,status:"open",due_date:newDue||null,engagement_id:engId,department:user.dept==="BOTH"?"TC":user.dept}).select("id").single();
if(saved?.id) setTasks(ts=>ts.map(t=>t.id===localTask.id?{...t,id:saved.id}:t));
}
}catch(e){console.warn("addTask:",e.message);}
};
...
<table style={{width:"100%",borderCollapse:"collapse"}}>
<thead><tr style={{background:C.warm50}}>{["","TAAK","PRIO","STATUS","DATUM","MGR"].map((h,i)=><th key={i} style={{padding:"9px 14px",textAlign:"left",fontSize:9,fontWeight:700,letterSpacing:"0.09em",color:C.secondary,textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
...
const sColor={pending:C.amber,overdue:C.red,completed:C.green};
const sBg={pending:C.amberBg,overdue:C.redBg,completed:C.greenBg};
const sLabel={pending:"OPENSTAAND",overdue:"ACHTERSTALLIG",completed:"VOLTOOID"};
const typeColor={upload:"#6366F1",approve:C.green,sign:C.amber,review:C.secondary};
const typeLabel={upload:"Upload",approve:"Goedkeuring",sign:"Ondertekening",review:"Beoordeling"};
useEffect(()=>{
setLoading(true);
supabase.from("client_actions")
.select("id,title,description,action_type,status,deadline,department")
.eq("department",dept)
.order("created_at",{ascending:false})
.then(({data})=>{
if(data) setActions(data.map(a=>({id:a.id,title:a.title,desc:a.description||"",type:a.action_type||"review",status:a.status||"pending",deadline:a.deadline?new Date(a.deadline).toLocaleDateString("nl-SR",{day:"2-digit",month:"short",year:"numeric"}):"—",dept:a.department})));
setLoading(false);
}).catch(()=>{ setActions(Object.values(CLIENT_ACTIONS_BY_ENG).flat()); setLoading(false); });
},[dept]);
const filtered=actions.filter(a=>{ const sOk=statusF==="ALL"||a.status===statusF; const qOk=!q||a.title.toLowerCase().includes(q.toLowerCase()); return sOk&&qOk; });
const markDone=(id)=>{
setActions(as=>as.map(a=>a.id===id?{...a,status:"completed"}:a));
supabase.from("client_actions").update({status:"completed"}).eq("id",id).catch(()=>{});
...
<button onClick={()=>setShowNew(true)} style={{display:"flex",alignItems:"center",gap:7,padding:"10px 16px",borderRadius:10,background:C.crimson,color:CREAM,border:"none",fontSize:11,fontWeight:700,cursor:"pointer",marginTop:4}}><Plus size={13}/> Nieuwe actie</button>
</div>
...
<div style={{width:38,height:38,borderRadius:10,background:`${k.color}20`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><k.Icon size={16} color={k.color}/></div>
<div><div style={{fontFamily:F.display,fontSize:28,fontWeight:600,color:k.color,lineHeight:1}}>{k.value}</div><div style={{fontSize:9,fontWeight:700,color:k.color,letterSpacing:"0.08em",textTransform:"uppercase",marginTop:2}}>{k.label}</div></div>
</div>
))}
</div>
...
{[["ALL","Alle"],["pending","Openstaand"],["overdue","Achterstallig"],["completed","Voltooid"]].map(([v,l])=>(<Pill key={v} label={l} active={statusF===v} onClick={()=>setStatusF(v)}/>))}
</div>
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden"}}>
{loading?(<div style={{padding:"32px 24px",textAlign:"center",color:C.secondary,fontSize:12}}>Laden…</div>):filtered.length===0?(
...
<table style={{width:"100%",borderCollapse:"collapse"}}>
<thead><tr style={{background:C.warm50}}>{["ACTIE","TYPE","DEADLINE","STATUS","BEHEREN"].map((h,i)=><th key={i} style={{padding:"10px 18px",textAlign:"left",fontSize:9,fontWeight:700,letterSpacing:"0.09em",color:C.secondary,textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
<tbody>{filtered.map(a=>(
<tr key={a.id} style={{borderTop:`1px solid ${C.border}`}}>
<td style={{padding:"13px 18px"}}><div style={{fontSize:13,fontWeight:600,color:C.text}}>{a.title}</div><div style={{fontSize:10,color:C.secondary,marginTop:2}}>{(a.desc||"").slice(0,60)}{(a.desc||"").length>60?"...":""}</div></td>
<td style={{padding:"13px 18px"}}><span style={{fontSize:9,fontWeight:700,background:`${typeColor[a.type]||C.secondary}18`,color:typeColor[a.type]||C.secondary,padding:"3px 9px",borderRadius:4,textTransform:"uppercase"}}>{typeLabel[a.type]||a.type}</span></td>
...
<td style={{padding:"13px 18px"}}><span style={{fontSize:9,fontWeight:700,background:sBg[a.status]||C.warm50,color:sColor[a.status]||C.secondary,padding:"3px 9px",borderRadius:4,textTransform:"uppercase"}}>{sLabel[a.status]||a.status}</span></td>
<td style={{padding:"13px 18px"}}>{a.status!=="completed"&&<button onClick={()=>markDone(a.id)} style={{padding:"5px 10px",borderRadius:7,background:C.greenBg,border:`1px solid ${C.green}30`,color:C.green,fontSize:10,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}><CheckCircle size={11}/> Voltooien</button>}</td>
</tr>
))}</tbody>
</table>
)}
...
{showNew&&(
<NewClientActionModal eng={{id:"standalone",name:"Nieuwe Actie",ref:dept+"-NIEUW",dept,client:"Selecteer cliënt"}} onClose={()=>setShowNew(false)} onCreated={a=>setActions(as=>[{...a,dept,desc:a.desc||"",deadline:a.deadline||"—"},...as])} showToast={showToast||(()=>{})}/>
)}
</div>
);
}
...
// Load all pending review items from DB
useEffect(()=>{
  const loadAll=async()=>{
    try{
      const items=[];
      ...
      (pendingDocs||[]).forEach(d=>items.push({
        ...
        sourceType:"document", reviewType:"document",
        typeLabel:"Document",
      }));
      ...
      (reviewEngs||[]).forEach(e=>items.push({
        ...
        sourceType:"engagement", reviewType:"engagement",
        typeLabel:"Engagement Review",
      }));
      // 3. Client actions with action_type = 'review' or status = 'pending'
      ...
      (reviewActions||[]).forEach(a=>items.push({
        ...
        sourceType:"client_action", reviewType:"client_action",
        typeLabel:"Cliëntactie Review",
      }));
      // Count today's verified/rejected items as "processed"
...
{[{l:t("pending"),v:docs.length,dark:false},{l:t("processedToday"),v:processedCount,dark:true}].map(s=>(
...
:docs.length===0?(
  <div style={{padding:"48px 24px",textAlign:"center"}}>
    ...
    &bull; Een engagement op <strong>&ldquo;In Review&rdquo;</strong> status wordt gezet<br/>
    &bull; Een cliëntactie van type <strong>&ldquo;Beoordelen&rdquo;</strong> wordt aangemaakt<br/>
    &bull; Een document wordt geüpload via Documentbeheer</div>
  </div>
):(
<table style={{width:"100%",borderCollapse:"collapse"}}>
<thead><tr style={{background:C.warm50}}>{["ITEM","TYPE","CLIËNT","DATUM","ACTIE"].map(h=><th key={h} style={{padding:"9px 19px",textAlign:"left",fontSize:9,fontWeight:700,letterSpacing:"0.09em",color:C.secondary,textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
<tbody>{docs.map(d=>(
<tr key={`${d.sourceType}-${d.id}`} style={{borderTop:`1px solid ${C.border}`}}>
<td style={{padding:"13px 19px"}}>
  <div style={{display:"flex",alignItems:"center",gap:9}}>
    <div style={{width:30,height:30,borderRadius:8,background:typeBgMap[d.reviewType]||C.crimsonFaint,display:"flex",alignItems:"center",justifyContent:"center"}}>
      {typeIconMap[d.reviewType]||<FileText size={13} color={C.crimson}/>}
    </div>
    <div><div style={{fontSize:13,fontWeight:600,color:C.text}}>{d.name}</div><div style={{fontSize:10,color:C.secondary}}>{d.dept}</div></div>
  </div>
</td>
<td style={{padding:"13px 19px"}}>
  <span style={{fontSize:9,fontWeight:700,padding:"3px 9px",borderRadius:20,background:typeBgMap[d.reviewType]||C.warm50,color:typeColorMap[d.reviewType]||C.secondary,textTransform:"uppercase"}}>{d.typeLabel}</span>
</td>
...
{[{title:"Nieuwe Uploads",desc:"Items worden live bijgewerkt vanuit de database.",c:C.amber}].map(a=>(
<div key={a.title} style={{padding:"10px 12px",borderRadius:9,borderLeft:`3px solid ${a.c}`,background:C.bg,marginBottom:7}}>
<div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:2}}>{a.title}</div>
<div style={{fontSize:11,color:C.secondary,lineHeight:1.5}}>{a.desc}</div>
</div>
))}
</div>
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,padding:"16px"}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
<div><div style={{fontSize:13,fontWeight:700,color:C.text}}>{t("queueLoad")}</div><div style={{fontSize:11,color:C.secondary}}>{docs.length>0?`${Math.min(100,docs.length*10)}%`:"0%"}</div></div>
</div>
...
<div style={{display:"flex",alignItems:"center",gap:9}}>{typeIconMap[reviewing.reviewType]||<FileText size={15} color={C.crimson}/>}<span style={{fontFamily:F.display,fontSize:16,fontWeight:600,color:C.text}}>{reviewing.name}</span></div>
...
<span style={{marginLeft:16,fontSize:12,color:C.secondary}}>Type:</span><span style={{fontSize:12,fontWeight:600,color:C.text}}>{reviewing.typeLabel}</span>
</div>
<div style={{display:"flex",gap:10,marginBottom:16}}>
{[{v:"verified",label:"Verifiëren",bg:C.green,Icon:CheckCircle},{v:"rejected",label:"Afwijzen",bg:C.red,Icon:X}].map(opt=>(
<button key={opt.v} onClick={()=>setDecision(opt.v)} style={{flex:1,padding:"11px",borderRadius:10,border:`2px solid ${decision===opt.v?opt.bg:C.border}`,background:decision===opt.v?opt.bg:"transparent",color:decision===opt.v?CREAM:C.text,fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s"}}>{opt.label}</button>
))}
</div>
    ...
    <div style={{overflowY:"auto",padding:"24px 26px",display:"flex",flexDirection:"column",gap:18}}>
      ...
      {user.dept==="BOTH"&&(
        <Field label="Afdeling" required>
          <div style={{display:"flex",gap:8}}>
            {["TC","FF"].map(d=>(
              <button key={d} onClick={()=>setDept(d)} style={{flex:1,padding:"10px",borderRadius:10,border:`2px solid ${dept===d?C.crimson:C.border}`,background:dept===d?C.crimsonFaint:"transparent",color:dept===d?C.crimson:C.secondary,fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s"}}>
      ...
      <Field label="Bedrijfslogo">
        ...
        <div
          ...
          style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",borderRadius:10,border:`2px dashed ${logoDragging?C.crimson:logoUrl?C.green:C.border}`,background:logoDragging?C.crimsonFaint:logoUrl?C.greenBg:C.bg,cursor:"pointer",transition:"all .15s"}}
        >
          {logoUrl?(
            <>
...
<table style={{width:“100%”,borderCollapse:“collapse”}}>
<thead><tr style={{background:C.warm50}}>{[t(“companyName”),t(“contactPerson”),“AFDELING”,“SECTOR”,t(“lifecycle”),””].map((h,i)=><th key={i} style={{padding:“9px 19px”,textAlign:“left”,fontSize:9,fontWeight:700,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{h}</th>)}</tr></thead>
<tbody>{list.map(c=>(
...
useEffect(()=>{
  supabase.from(“leads”)
    .select(“id,company_name,contact_name,estimated_value,stage,department,assigned_to”)
    .order(“created_at”,{ascending:false})
    .then(({data})=>{
      if(data?.length) setLeadsData(data.map(l=>({
        id:l.id,name:l.company_name,contact:l.contact_name,
        value:l.estimated_value||0,stage:l.stage||“new”,
        dept:l.department,rep:“—”,
      })));
    }).catch(()=>{});
},[]);
const leads=leadsData.filter(l=>{ const dOk=(user.dept===“BOTH”||l.dept===user.dept)&&(deptF===“ALL”||l.dept===deptF); const qOk=!q||l.name.toLowerCase().includes(q.toLowerCase()); return dOk&&qOk; });
return(
<div>
...
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:“hidden”}}>
<table style={{width:“100%”,borderCollapse:“collapse”}}>
<thead><tr style={{background:C.warm50}}>{[t(“company”),“AFDELING”,t(“stage”),t(“value”),t(“advisor”)].map(h=><th key={h} style={{padding:“9px 18px”,textAlign:“left”,fontSize:9,fontWeight:700,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{h}</th>)}</tr></thead>
<tbody>{leads.map(l=>(
...
<td style={{padding:“12px 18px”}}><DeptTag dept={l.dept}/></td>
<td style={{padding:“12px 18px”}}><Badge label={sL[l.stage]||l.stage} color={sC[l.stage]||C.secondary} bg={C.warm50}/></td>
<td style={{padding:“12px 18px”,fontFamily:F.display,fontSize:15,fontWeight:600,color:C.text}}>SRD {l.value.toLocaleString()}</td>
<td style={{padding:“12px 18px”}}><Avatar initials={l.rep} size={26} bg={l.dept===“TC”?C.crimson:C.taupe}/></td>
</tr>
))}</tbody>
</table>
</div>
...
useEffect(()=>{
  supabase.from(“documents”)
    .select(“id,name,file_type,visibility,review_status,department,uploaded_at”)
    .order(“uploaded_at”,{ascending:false})
    .then(({data})=>{
      if(data?.length) setAllDocs(data.map(d=>({
        id:d.id,name:d.name,
        type:d.file_type?d.file_type.toUpperCase():“PDF”,
        visibility:d.visibility||“internal”,
        dept:d.department,
        status:d.review_status||“pending”,
        date:d.uploaded_at?new Date(d.uploaded_at).toLocaleDateString(“nl-SR”,{day:“2-digit”,month:“short”}):“—”,
        size:“—”,uploadedBy:“?”,engagement:“—”,
      })));
    }).catch(()=>{});
},[]);
const docs=allDocs.filter(d=>{ const vOk=visF===“ALL”||d.visibility===visF; const sOk=user.dept===“BOTH”||d.dept===user.dept; const qOk=!q||d.name.toLowerCase().includes(q.toLowerCase()); return vOk&&sOk&&qOk; });
...
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:“hidden”}}>
<table style={{width:“100%”,borderCollapse:“collapse”}}>
<thead><tr style={{background:C.warm50}}>{[“DOCUMENT”,“AFDELING”,“DATUM”,“ZICHTBAARHEID”,“STATUS”,””].map((h,i)=><th key={i} style={{padding:“9px 16px”,textAlign:“left”,fontSize:9,fontWeight:700,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{h}</th>)}</tr></thead>
<tbody>{docs.map(d=>(
<tr key={d.id} onClick={()=>setSelected(d)} style={{borderTop:`1px solid ${C.border}`,cursor:“pointer”}}>
<td style={{padding:“12px 16px”}}><div style={{display:“flex”,alignItems:“center”,gap:9}}><div style={{width:30,height:30,borderRadius:7,background:d.dept===“TC”?C.crimsonFaint:”#F0EDE8”,display:“flex”,alignItems:“center”,justifyContent:“center”,flexShrink:0}}>{typeIconCmp[d.type]||<FileText size={14} color={C.secondary}/>}</div><div><div style={{fontSize:13,fontWeight:600,color:C.text}}>{d.name}</div><div style={{fontSize:10,color:C.secondary}}>{d.size}</div></div></div></td>
<td style={{padding:“12px 16px”}}><DeptTag dept={d.dept}/></td>
<td style={{padding:“12px 16px”,fontSize:11,color:C.secondary}}>{d.date}</td>
<td style={{padding:“12px 16px”}}><VisChip vis={d.visibility}/></td>
<td style={{padding:“12px 16px”}}><ReviewChip status={d.status}/></td>
<td style={{padding:“12px 16px”}}><button onClick={e=>{e.stopPropagation();showToast(`${d.name} gedownload`);}} style={{padding:“5px 10px”,borderRadius:7,background:C.bg,border:`1px solid ${C.border}`,fontSize:10,fontWeight:700,color:C.secondary,cursor:“pointer”}}><Download size={11}/></button></td>
</tr>
))}</tbody>
</table>
</div>
...
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,overflow:“hidden”}}>
<table style={{width:“100%”,borderCollapse:“collapse”}}>
<thead><tr style={{background:C.warm50}}>{[“REF”,“CLIËNT”,“AFDELING”,t(“amount”),t(“dueCol”),“STATUS”,“QBO”].map(h=><th key={h} style={{padding:“9px 16px”,textAlign:“left”,fontSize:9,fontWeight:700,letterSpacing:“0.09em”,color:C.secondary,textTransform:“uppercase”}}>{h}</th>)}</tr></thead>
...
<div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,padding:“18px 20px”,display:“flex”,flexDirection:“column”}}>
...
showToast(`${dept==="TC"?"Project":"Dossier"} "${name}" aangemaakt`);
onClose();
// Try to persist to Supabase in background
if(company) {
createEngagement({name,dept,phase,status,ref,company_id:company.id,deadline}).catch(e=>console.warn(“createEngagement:”,e.message));
}
};

return(
<div style={{position:“fixed”,inset:0,background:“rgba(58,46,40,.55)”,backdropFilter:“blur(4px)”,display:“flex”,alignItems:“center”,justifyContent:“center”,zIndex:2000,padding:20}} onClick={onClose}>
<div onClick={e=>e.stopPropagation()} className=“fu” style={{background:C.surface,borderRadius:20,width:560,maxWidth:“95vw”,boxShadow:“0 40px 100px rgba(58,46,40,.3)”,overflow:“hidden”}}>
<div style={{padding:“20px 24px”,borderBottom:`1px solid ${C.border}`,display:“flex”,justifyContent:“space-between”,alignItems:“center”,background:C.crimsonFaint}}>
<div style={{display:“flex”,alignItems:“center”,gap:10}}>
<div style={{width:36,height:36,borderRadius:9,background:C.crimson,display:“flex”,alignItems:“center”,justifyContent:“center”}}><Plus size={18} color={CREAM}/></div>
<div>
<div style={{fontFamily:F.display,fontSize:18,fontWeight:600,color:C.text}}>Nieuw {dept===“TC”?“Project”:“Dossier”}</div>
<div style={{fontSize:10,color:C.secondary}}>Vul alle verplichte velden in om aan te maken</div>
</div>
</div>
...
{user.dept===“BOTH”&&(
<div>
<div style={{fontSize:10,fontWeight:700,color:C.secondary,letterSpacing:“0.09em”,textTransform:“uppercase”,marginBottom:8}}>AFDELING *</div>
<div style={{display:“flex”,gap:8}}>
{[“TC”,“FF”].map(d=>(
...
{/* Name */}
<div>
<div style={{fontSize:10,fontWeight:700,color:C.secondary,letterSpacing:“0.09em”,textTransform:“uppercase”,marginBottom:6}}>{dept===“TC”?“PROJECTNAAM”:“DOSSIERNAAM”} *</div>
<input value={name} onChange={e=>setName(e.target.value)} placeholder={dept===“TC”?“Bijv. Staatsolie Q3 Analyse”:“Bijv. Hakrinbank Fiscale Review”} style={{width:“100%”,padding:“10px 14px”,borderRadius:9,border:`1.5px solid ${name?C.crimson:C.border}`,fontSize:13,outline:“none”,boxSizing:“border-box”}}/>
</div>
{/* Client */}
<div>
<div style={{fontSize:10,fontWeight:700,color:C.secondary,letterSpacing:“0.09em”,textTransform:“uppercase”,marginBottom:6}}>CLIËNT *</div>
<select value={client} onChange={e=>setClient(e.target.value)} style={{width:“100%”,padding:“10px 14px”,borderRadius:9,border:`1.5px solid ${client?C.crimson:C.border}`,fontSize:13,outline:“none”,cursor:“pointer”,background:C.surface,boxSizing:“border-box”}}>
...
<div style={{display:“grid”,gridTemplateColumns:“1fr 1fr”,gap:12}}>
<div>
<div style={{fontSize:10,fontWeight:700,color:C.secondary,letterSpacing:“0.09em”,textTransform:“uppercase”,marginBottom:6}}>STARTFASE</div>
<select value={phase} onChange={e=>setPhase(e.target.value)} style={{width:“100%”,padding:“10px 14px”,borderRadius:9,border:`1px solid ${C.border}`,fontSize:12,outline:“none”,cursor:“pointer”,background:C.surface}}>
{phases.map(p=><option key={p} value={p}>{p}</option>)}
</select>
</div>
<div>
<div style={{fontSize:10,fontWeight:700,color:C.secondary,letterSpacing:“0.09em”,textTransform:“uppercase”,marginBottom:6}}>STATUS</div>
<select value={status} onChange={e=>setStatus(e.target.value)} style={{width:“100%”,padding:“10px 14px”,borderRadius:9,border:`1px solid ${C.border}`,fontSize:12,outline:“none”,cursor:“pointer”,background:C.surface}}>
{ENGAGEMENT_STATUSES.map(s=><option key={s} value={s}>{s}</option>)}
</select>
</div>
</div>
{/* Deadline */}
<div>
<div style={{fontSize:10,fontWeight:700,color:C.secondary,letterSpacing:“0.09em”,textTransform:“uppercase”,marginBottom:6}}>DEADLINE</div>
<input type=“date” value={deadline} onChange={e=>setDeadline(e.target.value)} style={{width:“100%”,padding:“10px 14px”,borderRadius:9,border:`1px solid ${C.border}`,fontSize:13,outline:“none”,boxSizing:“border-box”}}/>
</div>
{/* Actions */}
<div style={{display:“flex”,gap:10,paddingTop:4}}>
<button onClick={submit} disabled={!name||!client} style={{flex:1,padding:“12px”,borderRadius:10,background:name&&client?C.crimson:C.mushroom,color:CREAM,border:“none”,fontSize:13,fontWeight:700,cursor:name&&client?“pointer”:“default”,display:“flex”,alignItems:“center”,justifyContent:“center”,gap:7}}>
<Plus size={15}/> {dept===“TC”?“Project”:“Dossier”} aanmaken
</button>
<button onClick={onClose} style={{padding:“12px 20px”,borderRadius:10,background:“transparent”,border:`1.5px solid ${C.border}`,color:C.text,fontSize:13,fontWeight:600,cursor:“pointer”}}>Annuleren</button>
</div>
</div>
...
```
  ...
  <div style={{display:"flex",gap:2,marginBottom:16,background:C.bg,borderRadius:12,padding:4,width:"fit-content"}}>
    {tabs.map(tb=>(
      <button key={tb.id} onClick={()=>setTab(tb.id)} style={{padding:"8px 16px",borderRadius:9,border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:7,background:tab===tb.id?C.surface:"transparent",color:tab===tb.id?C.text:C.secondary,fontWeight:tab===tb.id?700:400,fontSize:12,boxShadow:tab===tb.id?"0 1px 4px rgba(0,0,0,.07)":"none",transition:"all .15s"}}>
        <tb.Icon size={13}/>{tb.label}
      </button>
    ))}
  </div>
...
return(
<button key={ch.id} onClick={()=>toggle(ch.id)} style={{display:“flex”,alignItems:“center”,gap:7,padding:“7px 14px”,borderRadius:9,border:`2px solid ${isSel?platformColor[ch.platform]:C.border}`,background:isSel?`${platformColor[ch.platform]}15`:“transparent”,color:isSel?platformColor[ch.platform]:C.secondary,fontSize:11,fontWeight:700,cursor:“pointer”,transition:“all .15s”}}>
...
<div onClick={onChange} style={{display:“flex”,alignItems:“center”,gap:14,padding:“13px 16px”,borderRadius:10,border:`1px solid ${val?C.crimson:C.border}`,background:val?C.crimsonFaint:C.bg,cursor:“pointer”,transition:“all .15s”,marginBottom:8}}>
<div style={{flex:1}}>
...
```
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {activeTab==="profile"&&(
        <>
          <div style={{background:C.surface,borderRadius:14,border:`1px solid ${C.border}`,padding:"22px 24px"}}>
...
// Try by auth UID first, fallback to email lookup
let profile=null;
const {data:p1}=await supabase.from(‘user_profiles’).select(’*’).eq(‘id’,data.user.id).single();
if(p1) profile=p1;
else {
const {data:p2}=await supabase.from(‘user_profiles’).select(’*’).eq(‘email’,data.user.email).single();
if(p2){
// Link auth UID to profile for future logins
await supabase.from(‘user_profiles’).update({id:data.user.id}).eq(‘email’,data.user.email);
profile=p2;
}
}
...
<LangCtx.Provider value={language}>
<GlobalStyles/>
<div style={{minHeight:“100vh”,background:“linear-gradient(135deg,#F5F1EC 0%,#EDE7E0 100%)”,display:“flex”,flexDirection:“column”}}>
<div style={{display:“flex”,justifyContent:“flex-end”,padding:“16px 24px”}}>
<div style={{display:“flex”,border:`1px solid ${C.border}`,borderRadius:7,overflow:“hidden”}}>
{[“NL”,“EN”].map(l=>(<button key={l} onClick={()=>setLanguage(l)} style={{padding:“4px 10px”,fontSize:11,fontWeight:700,border:“none”,cursor:“pointer”,background:language===l?C.crimson:“transparent”,color:language===l?CREAM:C.secondary}}>{l}</button>))}
</div>
</div>
<div style={{flex:1,display:“flex”,alignItems:“center”,justifyContent:“center”,padding:“20px”}}>
<div style={{display:“grid”,gridTemplateColumns:“1fr 1fr”,gap:0,maxWidth:900,width:“100%”,borderRadius:20,overflow:“hidden”,boxShadow:“0 24px 80px rgba(58,46,40,.18)”}}>
{/* Left panel */}
<div style={{background:C.espresso,padding:“48px 40px”,color:CREAM,display:“flex”,flexDirection:“column”,gap:16}}>
<div style={{fontFamily:F.display,fontSize:11,fontWeight:600,letterSpacing:“0.2em”,color:C.taupeLight,textTransform:“uppercase”,marginBottom:8}}>Dual-Department</div>
<h1 style={{fontFamily:F.display,fontSize:32,fontWeight:600,color:CREAM,margin:0,lineHeight:1.2}}>Business Suite<br/>Intelligence</h1>
<div style={{display:“flex”,flexDirection:“column”,gap:12,marginTop:16}}>
{[{logo:“TC”,name:“Tactigent”,sub:“STRATEGISCHE OPERATIES”},{logo:“FF”,name:“Fiscal Fuse”,sub:“FINANCIËLE ANALYSE”}].map(d=>(
<div key={d.logo} style={{background:“rgba(255,255,255,.07)”,borderRadius:12,padding:“14px 16px”,display:“flex”,alignItems:“center”,gap:12}}>
<div style={{width:36,height:36,borderRadius:8,background:d.logo===“TC”?C.crimson:C.taupe,display:“flex”,alignItems:“center”,justifyContent:“center”,color:CREAM,fontWeight:700,fontSize:11}}>{d.logo}</div>
<div><div style={{fontSize:14,fontWeight:700,color:CREAM}}>{d.name}</div><div style={{fontSize:9,color:C.taupeLight,letterSpacing:“0.1em”}}>{d.sub}</div></div>
</div>
))}
</div>
<div style={{marginTop:“auto”,display:“flex”,alignItems:“center”,gap:6}}><Shield size={11} color={C.taupeLight}/><span style={{fontSize:9,color:C.taupeLight,letterSpacing:“0.1em”}}>ENCRYPTED EXECUTIVE GATEWAY</span></div>
</div>
{/* Right panel */}
<div style={{background:C.surface,padding:“48px 40px”,display:“flex”,flexDirection:“column”,justifyContent:“center”}}>
<div style={{fontFamily:F.display,fontSize:26,fontWeight:600,color:C.text,marginBottom:6}}>{t(“welcome”)}</div>
<div style={{fontSize:12,color:C.secondary,marginBottom:28}}>Voer uw gegevens in om toegang te krijgen.</div>
<div style={{marginBottom:14}}>
<div style={{fontSize:9,fontWeight:700,color:C.secondary,letterSpacing:“0.1em”,textTransform:“uppercase”,marginBottom:6}}>{t(“email”)}</div>
<div style={{position:“relative”}}><Mail size={13} style={{position:“absolute”,left:12,top:“50%”,transform:“translateY(-50%)”,color:C.secondary}}/><input value={email} onChange={e=>setEmail(e.target.value)} placeholder={t(“emailPlaceholder”)} style={{width:“100%”,padding:“10px 12px 10px 36px”,borderRadius:9,border:`1.5px solid ${C.border}`,fontSize:12,outline:“none”,boxSizing:“border-box”}}/></div>
</div>
<div style={{marginBottom:20}}>
<div style={{display:“flex”,justifyContent:“space-between”,marginBottom:6}}><span style={{fontSize:9,fontWeight:700,color:C.secondary,letterSpacing:“0.1em”,textTransform:“uppercase”}}>{t(“password”)}</span><span style={{fontSize:9,fontWeight:700,color:C.crimson,cursor:“pointer”}}>{t(“forgotPw”)}</span></div>
<div style={{position:“relative”}}><Lock size={13} style={{position:“absolute”,left:12,top:“50%”,transform:“translateY(-50%)”,color:C.secondary}}/><input type=“password” value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key===“Enter”&&tryLogin()} placeholder=”••••••••” style={{width:“100%”,padding:“10px 12px 10px 36px”,borderRadius:9,border:`1.5px solid ${C.border}`,fontSize:12,outline:“none”,boxSizing:“border-box”}}/></div>
</div>
{err&&<div style={{fontSize:11,color:C.secondary,marginBottom:14,padding:“8px 12px”,borderRadius:8,background:C.warm50,borderLeft:`3px solid ${C.amber}`}}>{err}</div>}
<button onClick={tryLogin} disabled={loading} style={{width:“100%”,padding:“12px”,borderRadius:10,background:C.crimson,color:CREAM,border:“none”,fontSize:12,fontWeight:700,cursor:“pointer”,marginBottom:20}}>{loading?t(“loggingIn”):t(“login”)} →</button>
<div style={{borderTop:`1px solid ${C.border}`,paddingTop:20}}>
<div style={{fontSize:9,fontWeight:700,color:C.secondary,letterSpacing:“0.1em”,textTransform:“uppercase”,marginBottom:10}}>{t(“demoSelect”)}</div>
<div style={{display:“flex”,flexDirection:“column”,gap:8}}>
{DEMO_USERS.map(u=>(
<button key={u.id} onClick={()=>onLogin(u)} style={{display:“flex”,alignItems:“center”,gap:10,padding:“10px 14px”,borderRadius:10,border:`1.5px solid ${C.border}`,background:C.bg,cursor:“pointer”,textAlign:“left”,transition:“all .15s”}}>
<div style={{width:30,height:30,borderRadius:“50%”,background:u.role===“client”?C.walnut:u.dept===“FF”?C.taupe:C.crimson,display:“flex”,alignItems:“center”,justifyContent:“center”,fontSize:10,fontWeight:700,color:CREAM,flexShrink:0,letterSpacing:“0.03em”}}>{u.avatar}</div>
<div><div style={{fontSize:12,fontWeight:700,color:C.text}}>{u.name}</div><div style={{fontSize:10,color:C.secondary}}>{u.title}</div></div>
</button>
))}
</div>
</div>
<div style={{marginTop:20,textAlign:“center”,fontSize:9,color:C.muted}}>Beheerd door Corporate IT</div>
</div>
</div>
</div>
</div>
      ...
      {current.id === "done" && (
        <div style={{ textAlign: "center", padding: "8px 0 16px" }}>
          <div style={{ display: "flex", justifyContent: "center", gap: 16, marginBottom: 20 }}>
            {[
              { Icon: UserCheck, label: "Identiteit\nGeverifieerd" },
